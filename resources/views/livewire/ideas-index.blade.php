    ...
    <div class="flex flex-col space-y-3 filters md:flex-row md:space-y-0 md:space-x-6">
        ...
        <div class="w-full md:w-1/3">
            <select wire:model="filter" name="other_filters" id="other_filters" class="w-full px-4 py-2 border-none rounded-xl">
                <option value="No Filter">Ingen Filtre</option>
                <option value="Top Voted">Top Jobs</option>
                @if(auth()->check()) {
                    <option value="My Ideas">Mine Jobs</option>
                }
                @endif
            </select>
        </div>
        <div class="relative w-full md:w-2/3">
            <input wire:model="search" type="search" placeholder="Søg" class="w-full px-4 py-2 pl-8 placeholder-gray-900 bg-white border-none rounded-xl">
    ...
    <div class="my-8 space-y-6 ideas-container">
        ...
        @empty
            <div class="mx-auto mt-12 w-70">
                <div class="mt-6 font-bold text-center text-gray-400">Din søgning gav intet resultat... <br> Prøv at søge efter noget andet.</div>
            </div>
        @endforelse
